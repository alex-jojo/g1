...
def compute_policy_loss(
    ...
    """
    print(f"Compute policy loss {old_log_probs.shape} {new_log_probs.shape} {advantages.shape} {response_mask.shape}")
    start_time = time()
    ...
    # print('sum', response_mask.sum())
    # print('Loss', masked_loss.sum(), masked_loss.sum(dim=-1))
    # print("Compute policy loss time", time() - start_time)
    # exit()
    return (masked_loss.sum(dim=-1) / response_mask.sum(dim=-1)).sum()
